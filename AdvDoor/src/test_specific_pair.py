...
def test_deserialize_model(param, args):
    ...
    data1 = Data(param)
    data1.load_data(is_add_channel=True) #clean data
    print('!!!Clean data to clean model!!!')
    model.predict(data1) #clean clean

    
    data = Data(param)
    data.load_data()
    data.gen_backdoor(model) #poisoned data
    print('!!!Poison data to clean model!!!')
    model.predict(data) #poisoned clean

    
    
    model.train(data) #poisoned model
    ...
    model2.predict(data) #poisoned poisoned 
    model2.predict(data1) #clean poisoned 
...
def test_resume_model(param, args):
    ...
    # Clean data to Poison model   
    if param.get_conf('model_prefix') == 'cifar':
        print('!!!Clean data to Poison model!!!')
        Data = CifarData
        data_clean = Data(param)
        data_clean.load_data(is_add_channel=True)
        model.predict(data_clean)
    ...
    print('!!!Poison data to Poison model!!!')
    model.predict(data)
    '''
    if args.spectral:
...
def test_clean_model(param, args):
    K.set_learning_phase(0)
    if param.get_conf('model_prefix') == 'cifar':
        Data = CifarData
    
    path = param.get_conf('model_path')
    if os.path.exists(path) and 'pkl' in path:
        with open(path, 'rb') as f:
            model = pickle.load(f)
        model.set_param(param)
        print('model load success')
    #Clean data to clean model
    data_clean = Data(param)
    data_clean.load_data(is_add_channel=True) #clean data
    model.predict(data_clean)

    #Poison data to clean model
    data = Data(param)
    data.load_data()
    data.gen_backdoor(model) #poisoned data
    model.predict(data)

def test_poison_model(param, args):
    K.set_learning_phase(0)
    with open(param.get_conf('model_path_backdoor'), 'rb') as f:
        model = pickle.load(f)
    model.param.set_conf('pert_path', param.get_conf('pert_path'))
    ...
    if model.param.get_conf('model_prefix') == 'cifar':
        data = CifarData(model.param)
    
    data.load_data()
    data.restore_backdoor(model) #poisoned data
    model.param.print_conf()
    print('model load success')
    #Poison data to Poison model
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cm', '--clean', action='store_true', help='running clean model')
    parser.add_argument('-pm', '--poison', action='store_true', help='running poison model')
    ...
    if args.clean:
        test_clean_model(param, args)
    elif args.poison:
        test_poison_model(param, args)
    elif args.resume:
        test_resume_model(param, args)
    elif args.data:
        test_data(param, args)
    else:
